...
SELECT   TO_CHAR(COUNT(*), '99,999') AS "Transaction Before Merge"
FROM     transaction;
...
FROM ( SELECT
	  ...
	  , TO_CHAR(SUM(t.transaction_amount) * 1.1, '$9,999,999.00') AS ten_plus
	  , TO_CHAR(SUM(t.transaction_amount) * 1.2, '$9,999,999.00') AS twenty_plus
	  , TO_CHAR(SUM(t.transaction_amount) * 0.1, '$9,999,999.00') AS ten_plus_less_b
	  , TO_CHAR(SUM(t.transaction_amount) * 0.2, '$9,999,999.00') AS twenty_plus_less_b
	  , EXTRACT(MONTH FROM t.transaction_date) AS sKey
	  FROM transaction t
